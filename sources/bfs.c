...
/*   Updated: 2019/03/22 13:10:47 by tle-dieu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_queue *enqueue(t_queue *queue, t_room *room, char **flow)
{
	...
	while (i < room->nb_links)
	{
		if (!room->links[i]->i && !flow[room->id][room->links[i]->id] && (!room->links[i]->flow || get_back(room->links[i], flow)) && (!room->flow || room->back))
		{
			/* ft_printf("name: %s back: %d\n", room->links[i]->name, room->back); */
